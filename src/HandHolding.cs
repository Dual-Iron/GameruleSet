...
namespace GameruleSet
{
    ...
    {
        static readonly WeakTable<Creature.Grasp, GraspData> graspData = new(_ => new());

        sealed class GraspData
        {
        ...
        {
            new Hook(typeof(BodyChunk).GetMethod("get_goThroughFloors"), get_goThroughFloors)
                .Apply();
        ...
        private void ScavengerAI_Update(On.ScavengerAI.orig_Update orig, ScavengerAI self)
        {
            ...
            for (int i = self.scavenger.grabbedBy.Count - 1; i >= 0; i--)
            {
                ...
                if (CanHoldHand(grasp.grabber) && grasp.grabber is Player p)
                {
                    ...
                    if (offset != default)
                        graspData[grasp].prevOffset = offset;
                    else
                        offset = graspData[grasp].prevOffset;
        ...
        private void SlugcatHand_Update(On.SlugcatHand.orig_Update orig, SlugcatHand self)
        {
            ...
            if (self.owner.owner is Player player)
            {
                ...
                {
                    var pos = graspData[player.grasps[self.limbNumber]].handPos;
                    if (pos != default)
                    {
                        self.mode = Limb.Mode.HuntAbsolutePosition;
